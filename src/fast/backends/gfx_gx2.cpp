...
#include "fast/backends/gx2_util.h"
...
static void gfx_gx2_set_zmode_decal(bool zmode_decal) {
    ...
    if (zmode_decal) {
        ...
        float SSDB = -2.0f;
        switch (Ship::Context::GetInstance()->GetConsoleVariables()->GetInteger("gZFightingMode", 0)) {
...
static void gfx_gx2_init(void) {
    ...
    main_framebuffer.color_buffer.surface.image =
        GfxWiiUAllocMem1(main_framebuffer.color_buffer.surface.imageSize,
    ...
    main_framebuffer.depth_buffer.surface.image =
        GfxWiiUAllocMem1(main_framebuffer.depth_buffer.surface.imageSize,
    ...
    depthReadBuffer.surface.image =
        GfxWiiUAllocMem1(depthReadBuffer.surface.imageSize, depthReadBuffer.surface.alignment);
...
void GfxGX2Shutdown() {
    if (has_foreground) {
        ...
        if (depthReadBuffer.surface.image) {
            GfxWiiUFreeMem1(depthReadBuffer.surface.image);
        ...
        for (auto& buffer : framebuffers) {
            if (buffer.texture.surface.image) {
                if (buffer.colorBufferMem1) {
                    GfxWiiUFreeMem1(buffer.texture.surface.image);
                } else {
            ...
            if (buffer.depth_buffer.surface.image) {
                if (buffer.depthBufferMem1) {
                    GfxWiiUFreeMem1(buffer.depth_buffer.surface.image);
                } else {
...
static void gfx_gx2_update_framebuffer_parameters(int fb, uint32_t width, uint32_t height, uint32_t msaa_level,
    ...
    buffer.depth_buffer.surface.image =
        GfxWiiUAllocMem1(buffer.depth_buffer.surface.imageSize, buffer.depth_buffer.surface.alignment);
    // fall back to mem2
    if (!buffer.depth_buffer.surface.image) {
        buffer.depth_buffer.surface.image =
    ...
    buffer.texture.surface.image =
        GfxWiiUAllocMem1(buffer.texture.surface.imageSize, buffer.texture.surface.alignment);
    // fall back to mem2
    if (!buffer.texture.surface.image) {
...
void GfxRenderingAPIGX2::UnloadShader(ShaderProgram* oldPrg) { gfx_gx2_unload_shader(oldPrg); }
void GfxRenderingAPIGX2::LoadShader(ShaderProgram* newPrg) { gfx_gx2_load_shader(newPrg); }
...
void GfxRenderingAPIGX2::SelectTexture(int tile, uint32_t textureId) { gfx_gx2_select_texture(tile, textureId); }
...
void GfxRenderingAPIGX2::SetDepthTestAndMask(bool depthTest, bool zUpdate) {
    gfx_gx2_set_depth_test_and_mask(depthTest, zUpdate);
}
...
void GfxRenderingAPIGX2::SetUseAlpha(bool useAlpha) { gfx_gx2_set_use_alpha(useAlpha); }
...
void GfxRenderingAPIGX2::CopyFramebuffer(int fbDstId, int fbSrcId, int srcX0, int srcY0, int srcX1, int srcY1,
    ...
    gfx_gx2_copy_framebuffer(fbDstId, fbSrcId, srcX0, srcY0, srcX1, srcY1, dstX0, dstY0, dstX1, dstY1);
}
void GfxRenderingAPIGX2::ClearFramebuffer(bool color, bool depth) { gfx_gx2_clear_framebuffer(color, depth); }
void GfxRenderingAPIGX2::ReadFramebufferToCPU(int fbId, uint32_t width, uint32_t height, uint16_t* rgba16Buf) {
    gfx_gx2_read_framebuffer_to_cpu(fbId, width, height, rgba16Buf);
}
void GfxRenderingAPIGX2::ResolveMSAAColorBuffer(int fbIdTarget, int fbIdSource) {
    gfx_gx2_resolve_msaa_color_buffer(fbIdTarget, fbIdSource);
}
...
void* GfxRenderingAPIGX2::GetFramebufferTextureId(int fbId) { return gfx_gx2_get_framebuffer_texture_id(fbId); }
void GfxRenderingAPIGX2::SelectTextureFb(int fbId) { gfx_gx2_select_texture_fb(fbId); }
void GfxRenderingAPIGX2::DeleteTexture(uint32_t textureId) { gfx_gx2_delete_texture(textureId); }
...
void GfxRenderingAPIGX2::SetSrgbMode() { gfx_gx2_enable_srgb_mode(); }
ImTextureID GfxRenderingAPIGX2::GetTextureById(int id) { return GfxGX2TextureForImGui(static_cast<uint32_t>(id)); }
